...
template <>
  __global__ 
void add_bias_gelu(half* out, const half* __restrict bias, int m, int n)
{
...
template <typename T>
__global__ 
void add_bias_relu(T* out, const T* __restrict bias, int m, int n)
{
  for(int id = blockIdx.x * blockDim.x + threadIdx.x; id < m * n; id += blockDim.x * gridDim.x)
  {
    T reg_bias = __ldg(&bias[id % n]);
...
template <>
  __global__ 
void add_bias_relu(half* out, const half* __restrict bias, int m, int n)
{
  half2* out_ptr = (half2*) out;
  const half2* bias_ptr = (half2*) bias;
...
void add_bias_act_kernelLauncher(T* out, const T* bias, int m, int n, ActivationType activation_type, cudaStream_t stream)
{
  ...
  if(activation_type == ActivationType::RELU)
    ...
    add_bias_gelu<T><<<grid, block, 0, stream>>>(out, bias, m, n / data_type_factor);
}
...
template void add_bias_act_kernelLauncher<float>(
  float* out, const float* bias, int m, int n, ActivationType activation_type, cudaStream_t stream);
...
template void add_bias_act_kernelLauncher<half>(
  half* out, const half* bias, int m, int n, ActivationType activation_type, cudaStream_t stream);
...
template void add_bias_input_kernelLauncher<half>(
  half* output,
  const half* bias,
  const half* input,
  const int m,
